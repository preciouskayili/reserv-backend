...
router.get("/reservations/:code", async (req, res) =>
  res.json(
    publicState(
      await workspaces.byCode(String(req.params.code)),
...
router.patch("/reservations/:code", async (req, res) => {
  ...
  res.json(
    publicState(
      await changeReservation(await workspaces.byCode(code), code, req.body),
      code,
    ),
  );
});
...
router.post("/businesses/:slug/bookings", async (req, res) => {
  ...
  res
    .status(201)
    .json({ booking, snapshot: publicState(snapshot, booking.code) });
});
