...
import { checkAndDispatchReminders, getCallSchedulerStatus } from "../services/callScheduler.js";
...
const triggerCallSchema = z.object({
  ...
  callType: z
    ...
    .default("reminder"),
  customPromptVariables: z.record(z.string(), z.union([z.string(), z.number(), z.boolean()])).optional(),
});
...
router.post("/webhook", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (callId) {
      ...
      console.log(`[Aethex Webhook] Updated call ${callId} status to: ${status}`);
    }
