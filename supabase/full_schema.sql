...
BEGIN
 ...
 SELECT coalesce(sum((p->>'amount')::numeric),0) INTO paid FROM jsonb_array_elements(coalesce(doc->'payments','[]')) p WHERE p->>'bookingId'=attempt.booking_id AND p->>'status'='approved';
 ...
 payment := jsonb_build_object('id',p_id::text,'bookingId',attempt.booking_id,'amount',p_amount::numeric/100,'method','gateway','status','approved','createdAt',now());
